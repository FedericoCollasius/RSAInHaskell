...
exDescifrado :: Integer -> Integer -> Integer
exDescifrado i m  | sonCoprimos i m && i <= m - 2 = i
                  | otherwise = exDescifrado (i + 1) m
...
--proximoPrimoMenorOIgualDesde funciona igual que la otra función solo que en caso de que el n que le pasen sea 
-- primo, va a devolver ese n. 
proximoPrimoMenorOIgualDesde :: Integer -> Integer 
proximoPrimoMenorOIgualDesde 2 = 2 
proximoPrimoMenorOIgualDesde n | esPrimo n = n
                               | otherwise = proximoPrimoMenorDesde (n - 1)
