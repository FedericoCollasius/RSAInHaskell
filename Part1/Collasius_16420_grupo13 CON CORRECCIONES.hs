-- Devolución: El TP está en general bien. Hay algunas correcciones para mejorar el código.
-- Reentrega: reescribir las funciones con correcciones.
...
-- RESPUESTA: La definición equivalente para el PTF la verán en la materia álgebra 1 (no el taller),
-- lo que no sé si es cierto es si se puede usar esa fórmula para definir "esCarmichael".
...
--CORRECCIÓN: no es necesario el "== True", la función sonCoprimos_aux ya es un Bool.
...
--OBSERVACIÓN: se puede usar "otherwise" en la última guarda. Incluso se pueden juntar
-- los últimos dos casos en uno solo así:
-- sonCoprimos_aux a b n | n > a || n > b = True
--                       | otherwise     = not (mod a n == 0 && mod b n == 0) && sonCoprimos_aux a b (n+1)
...
--CORRECCIÓN: Estás escribiendo de una forma rebuscada lo siguiente:
...
--CORRECCIÓN: no es necesario utilizar el contador. Podés hacer que la función misma te devuelva
-- el valor que estás acumulando. De esta forma el código es más declarativo y podrías ponerle un
-- nombre más claro, por ejemplo "cantidad3PseudoprimosEntre" ya que "cantidad3PseudoprimosEntre n m" te
-- devuelve la cantidad de números que son 3-pseudoprimos entre n y m.
-- Mas aún, no hace falta usar una función auxiliar, se puede hacer recursión directamente,
-- definiendo "cantidad3Pseudoprimos n" en términos de "cantidad3PseudoPrimos (n-1)".
...
--CORRECCIÓN: no es necesario el "== True", y el "== False" se puede evitar utilizando "not".
-- De hecho podés evitar incluso eso utilizando "otherwise" en la última guarda.

es3Pseudoprimo :: Integer -> Bool
es3Pseudoprimo n | n > 1 && not (esPrimo n) && mod ((3^(n-1)) - 1) n == 0 = True
                 | otherwise                                              = False

--CORRECCIÓN: ver la corrección de "es2Pseudoprimo".
...
--OBSERVACIÓN: si bien esto resuelve el problema, es muy iterativo. Esto hace que sea más difícil
-- de entender porque para ver si es correcto uno tiene que "ejecutarla" en la cabeza.
-- Se puede implementar la función "kesimo2y3Pseudoprimo" de forma recursiva:

-- kesimo2y3Pseudoprimo 0 = 1
-- kesimo2y3Pseudoprimo k = primer2y3PseudoprimoDesde (kesimo2y3Pseudoprimo (k-1))

-- donde "primer2y3PseudoprimoMayor k" devuelve el primer número mayor a k que es
-- 2-pseudoprimo y 3-pseudoprimo.
...
--OBSERVACIÓN: la primer guarda tiene más sentido que esté en la función original porque no forma
-- parte de la recursión:
--esCarmichael n = n /= 1 && not (esPrimo n) && esCarmichael_aux2 n 1
-- Además, la función auxiliar se puede escribir más concisa:
-- esCarmichael_aux2 n m = n == m || (not (sonCoprimos n m) || esAPseudoprimo n m) && esCarmichael_aux2 n (m + 1)
-- Notar que ahora a la función auxiliar le podemos dar un nombre más declarativo, por ejemplo: "esAPseudoprimoSiCoprimoDesde".
...
-- OBSERVACIÓN: Parecería funcionar pero no sirve para el TP porque no cumple con el enunciado.
-- Para ser válido tendrías que demostrar que es equivalente.
